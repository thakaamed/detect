...
(function(jQuery, window, document, undfined) {
    ...
    var DragZoom = function(ele, opt) {
        ...
        this.defaults = {
            ...
            speed: 0.7,
            scope: null,
            onWheelStart: function(events) {
                console.log("zoom events", events)
            },
            onWheelEnd: function(events) {
                console.log("end çalıştı mı")
            },
    ...
    DragZoom.prototype = {
        Init: function() {
            ...
            $(document).on('mousemove', function(e) {
                ...
                self.move(e, self);
                // return false;
                // return false input rangelari bozuyor
            }).on('mouseup', function(e) {
                self.end(e, self);
                // return false;
            });
        ...
        start: function(ev, self) {
            console.log("did drag")
            
            if(typeof resetPointSpecify === 'function') {
                resetPointSpecify()
            }

			if(dragObjectMove == false){
				if(rectangle_state == false && polygon_state == false && circleMarker_state == false && !$("#editMode").prop("checked") && !$("#editPicture").prop("checked") && !calibrationsState && !impCrwEditModeStatus){
					self.isMoved = true;

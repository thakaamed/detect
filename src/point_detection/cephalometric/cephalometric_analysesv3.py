...
from src.cases.point_detection.cephalometric.src.configv3 import Config
...
class CephalometricAnalysis():
    ...
    def get_additional_points(self):
        ...
        self.analysis_points["Mx1c processed point"] = self.find_projected_point(self.analysis_points["Mx1c"],
                                                                                 self.planes["A-Pg plane"])
    ...
    def get_result_as_json(self):
        ...
        self.analysis_report_dict["results"]["planes"] = {
            ...
            Config.planes.items()}
        print(self.analysis_report_dict["results"]["planes"])
        self.analysis_report_dict["results"]["angle_planes"] = {
            angle: [self.analysis_report_dict["results"]["planes"][Config.angles[angle]["angle_planes"][0]],
                    self.analysis_report_dict["results"]["planes"][Config.angles[angle]["angle_planes"][1]]] for angle
            in Config.angles}
        self.analysis_report_dict["results"]["measurement_points"] = {measurement: [
            [content["coordinates"] for content in self.analysis_report_dict["results"]["cephalometric_landmarks"] if
             content["label"] == Config.measurements[measurement]["measurement_points"][0]][0],
            [content["coordinates"] for content in self.analysis_report_dict["results"]["cephalometric_landmarks"] if
             content["label"] == Config.measurements[measurement]["measurement_points"][1]][0]] for measurement in
                                                                      Config.measurements}
        self.analysis_report_dict["results"]["ratio_measurements"] = {ratio: Config.ratios[ratio]["ratio_heights"] for
                                                                      ratio in Config.ratios}
